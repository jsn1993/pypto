...
- **CPU 函数**（标量 + OpenMP）→ 使用多态向量类型的 C 代码 → gcc → `.so` → ctypes FFI
...
- **CodeGen**：三个后端 — PTO codegen（InCore → `.pto` MLIR，运行在 AICore）、Orchestration codegen（→ C++，运行在 AICPU）和 CPU codegen（→ C with OpenMP）
